...
# 2. Regenerate the full dataset (deterministic, seeded)
python scripts/generate_datasets.py --out data/ --users 50000 --seed 42

# 3. Explore the business questions in SQL (BigQuery dialect)
#    see sql/analysis/*.sql

# 4. (optional) Build the dbt models
cd dbt/streaming && dbt build
```

The full fact table (~1.2M rows / 63 MB) is regenerated by the script and is **not** committed;
a 100k-row sample lives in [`data/sample/`](data/sample/) so the repo is clone-and-run.
